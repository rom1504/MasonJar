  ...
  query.findOne(function (err, day_vote) {
    ...
    if(day_vote) {
      ...
      if(day_vote.votes.length / options.CONNECTED_PLAYERS.count >= VOTES_FOR_DAY) {
        cmd.whisper(payload.player, `You voted for day successfully.`);
        ...
        if(day_vote.votes.indexOf(payload.player) > -1) {
          cmd.whisper(payload.player, `You've already voted for day.  Percent is at ${day_vote.votes.length / options.CONNECTED_PLAYERS.count}, ${VOTES_FOR_DAY} or above needed.`);
        }else {
          day_vote.votes.push(payload.player);

          cmd.whisper(payload.player, `You voted for day. Percent is at ${day_vote.votes.length / options.CONNECTED_PLAYERS.count}, ${VOTES_FOR_DAY} or above needed.`);
